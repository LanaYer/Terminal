...
uses Unit1, Unit5, Unit3, Unit4, Unit6, Select_Operator;
...
Button1.Visible:=False;
 ...
 //Unit3.Form3.Show;
  Unit5.Form5.Show;
  Form2.Hide;
  SelectOperator.Timer3.Enabled:=True;


  AssignFile(F_sumOfDay, 'Sum.txt');
  Rewrite(F_sumOfDay);
  WriteLn(F_sumOfDay,IntToStr(SumOfDate));
  CloseFile(F_sumOfDay);

  Button2.Enabled:= true;



                                 ////  *** Записываем инкасацию ***
  {                             ///
AssignFile(F_sumOfDay, 'Sum.txt');
 if not(FileExists('Sum.txt')) then Rewrite(F_sumOfDay);      // если по ошибке удалят наш файл, то мы его пересоздадим

 F_sum_st.Create;
 try
   F_sum_st.LoadFromFile('Sum.txt');

   if(F_sum_st.Count >1)then
   begin
   F_sum_st[F_sum_st.Count -1]:= IntToStr(SumOfDATE);
   F_sum_st.SaveToFile('Sum.txt');
   end
   else
    //
 finally
...
begin
 ...
 begin
AssignFile(F_sumOfDay, 'Sum.txt');
 if not(FileExists('Sum.txt')) then
 begin
  Rewrite(F_sumOfDay);
  WriteLn(F_sumOfDay,'0');
  CloseFile(F_sumOfDay);
  SumofDate:=0;
 end
else
  begin
    reset(F_sumOfDay);
    Read(F_sumOfDay,i_sum);
    SumOfDate:=i_sum;
    CloseFile(F_sumOfDay);
    //ShowMessage('Считана сумма  '+IntToStr(SumOfDate));
   end;
